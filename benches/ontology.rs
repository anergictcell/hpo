...
fn build_ontology_benchmark(c: &mut Criterion) {
    ...
    File::open("tests/ontology.hpo").expect("ontology.hpo cannot be opened").read_to_end(&mut bytes).expect("cannot read ontology.hpo");
    c.bench_function("build ontology", |b| {
        b.iter(|| Ontology::from_bytes(black_box(&bytes[..])).expect("requires valid bytes").len())
    });
}
