...
impl StandardCombiner {
    ...
    fn fun_sim_max(&self, m:&Matrix<f32>) -> f32 {
        ...
        (row_maxes.iter().sum::<f32>() / rows).max(
            col_maxes.iter().sum::<f32>() / cols
        )
    }
    ...
    fn bwa(&self, m:&Matrix<f32>) -> f32 {
        ...
        (row_maxes.iter().sum::<f32>() + col_maxes.iter().sum::<f32>()) /
        (rows + cols)
    }
...
fn usize_to_f32(n: usize) -> f32 {
    <usize as TryInto<u16>>::try_into(n).expect("Matrix too large").into()
}